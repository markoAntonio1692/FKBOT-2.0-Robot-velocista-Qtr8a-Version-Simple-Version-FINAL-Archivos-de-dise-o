...
void QTRSensors::readCalibrated(unsigned int *sensor_values, unsigned char readMode,int flanco_sens)
{
    ...
    for(i=0;i<_numSensors;i++)
    {
        ...
        if(x < 0)
            ...
            x = 1000;
        sensor_values[i] = x + flanco_sens;  /// +500 fondo negro
    }
...
int QTRSensors::readLine(unsigned int *sensor_values,unsigned char readMode, unsigned char white_line,int flanco_sens, int keep_track, int noise_threshold )
{
